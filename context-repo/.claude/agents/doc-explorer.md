...
name: doc-explorer
description: >-
  Subagente read-only de mapeamento do grafo de documentação. Use para, dado um doc ou ID
  alterado, listar todos os `children`/`related` afetados (inclusive cross-repo via `ID@repo`),
  apontar quebras de integridade (links assimétricos, parents/children faltando, termos de
  domínio fora do GLO) e produzir a lista de propagação. Não escreve nada. Despachado pelo
  orquestrador `cascade` no passo de Mapa de Impacto.
model: haiku
